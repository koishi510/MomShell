# MomShell Documentation

Welcome to the MomShell documentation. This index will help you find the information you need.

## Quick Links

| Document | Description |
|----------|-------------|
| [Features](features.md) | Detailed feature descriptions and use cases |
| [Getting Started](getting-started.md) | Quick start guide for new users |
| [Development](development.md) | Development environment setup |
| [Deployment](deployment.md) | Docker deployment guide |
| [Configuration](configuration.md) | Environment variables reference |
| [Architecture](architecture.md) | Technical architecture overview |

## For New Users

1. **[Getting Started](getting-started.md)** - Install and run MomShell in minutes
2. **[Features](features.md)** - Explore what MomShell can do for you

## For Developers

1. **[Development](development.md)** - Set up your development environment
2. **[Architecture](architecture.md)** - Understand the codebase structure
3. **[Contributing](../CONTRIBUTING.md)** - Contribution guidelines

## For Deployment

1. **[Deployment](deployment.md)** - Deploy with Docker
2. **[Configuration](configuration.md)** - Configure environment variables

---

[Back to main README](../README.md)
