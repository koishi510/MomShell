...
1. **Select Python Interpreter**:
   ...
   - Choose the option containing `backend/.venv` (usually marked as Recommended)
...
# Docker deployment
make docker-up     # Build and start containers (from deploy/)
...
**Backend (Python)** - Files located in `backend/`:
...
- **Conflict Resolution**: If `uv.lock` or `requirements.txt` conflicts after `git pull`, do not manually merge. Run `cd backend && uv lock && uv export > requirements.txt` to regenerate, then commit.
...
- **Q: VS Code showing code errors?**
  - A: Verify the Python interpreter (bottom-right corner) is set to the `backend/.venv` virtual environment, not the system Python.
