      ...
      (keypoints: Record<string, Keypoint>): Record<string, Keypoint> => {
        ...
        for (const [idx, point] of Object.entries(keypoints)) {
          ...
          if (prevPoint && prevPoint.visibility > 0.3) {
            smoothed[idx] = {
              x:
                point.x * (1 - smoothingFactor) + prevPoint.x * smoothingFactor,
              y:
                point.y * (1 - smoothingFactor) + prevPoint.y * smoothingFactor,
    ...
    const drawSkeleton = useCallback(
      (
        ...
        const highlightStrokeColor =
          SKELETON_COLORS[highlightColor] || SKELETON_COLORS.pink;
        ...
        for (const [startIdx, endIdx] of POSE_CONNECTIONS) {
          ...
          if (start && end && start.visibility > 0.5 && end.visibility > 0.5) {
            const isHighlighted =
              highlightedIndices.has(startIdx) &&
              highlightedIndices.has(endIdx);
        ...
        for (const [idx, point] of Object.entries(smoothed)) {
          if (point.visibility > 0.5) {
            ...
            const pointColor = isHighlighted
              ? highlightStrokeColor
              : strokeColor;
...
export function PoseQualityIndicator({
  score,
  size = "md",
}: PoseQualityIndicatorProps) {
  const sizeClasses = {
    sm: "w-8 h-8 text-xs",
