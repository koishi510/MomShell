...
export default function ReplyCard({
  ...
  const channelColors =
    CHANNEL_COLORS[answer.question.channel as keyof typeof CHANNEL_COLORS] ||
    CHANNEL_COLORS.experience;
      ...
      >
        ...
        <div className="mb-4 pb-4 border-b border-stone-100/80">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs text-stone-400">回复了</span>
        ...
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          <span
            className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${statusConfig[answer.status]?.color || "bg-stone-100 text-stone-600"}`}
          >
        ...
        <footer className="flex items-center justify-between pt-4 border-t border-stone-100/80">
          <div className="flex items-center gap-5">
            {/* 点赞 */}
            <div
              className={`flex items-center gap-1.5 ${answer.is_liked ? "text-rose-500" : "text-stone-400"}`}
            >
        ...
        <div
          ...
          title={
            answer.question.channel === "professional" ? "专业频道" : "经验频道"
          }
        />
      </motion.div>
    </motion.article>
