...
export default function ProfilePage() {
  ...
  const isProfessional = profile
    ? professionalRoles.includes(profile.role)
    : false;
  ...
  const handleSave = async () => {
    if (!editNickname.trim()) {
      ...
      setTimeout(
        () => setSaveMessage({ show: false, success: true, text: "" }),
        3000,
      );
      return;
    }
            ...
            >
              ...
              <div className="bg-white/70 backdrop-blur-sm rounded-3xl p-6 border border-stone-100/50">
                ...
                <div className="space-y-2">
                  ...
                  {profile.role === "admin" && (
                    <QuickLink
                      href="/community/admin/certifications"
                      icon="🛡️"
                      label="认证审核"
                    />
                  )}
...
function StatCard({
  ...
  const content = (
    <div
      className={`p-4 rounded-2xl bg-stone-50/80 ${href ? "hover:bg-stone-100/80 transition-colors cursor-pointer" : ""}`}
    >
