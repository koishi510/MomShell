...
export default function RehabPage() {
  ...
  const [skeletonColor, setSkeletonColor] = useState<
  ...
  const fetchProgress = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${getApiBase()}/api/v1/progress/${effectiveUserId}/summary`,
        { headers },
      );
      const data = await response.json();
  ...
  const fetchAchievements = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${getApiBase()}/api/v1/progress/${effectiveUserId}/achievements`,
        { headers },
      );
      const data = await response.json();
        ...
        case "state":
          if (data.data) {
            ...
            if (stateData.progress) {
              const newPhase =
                PHASE_NAMES[stateData.progress.current_phase] ||
                stateData.progress.current_phase;
          ...
          if (data.keypoints) {
            const color = (data.skeleton_color || "white") as
              | "green"
              | "yellow"
              | "red"
              | "white";
          ...
          <AnimatePresence mode="wait">
            {currentView === "exercises" && (
              <section key="exercises">
                <h2 className="text-2xl font-medium text-stone-700 mb-6">
                  康复动作
                </h2>
          ...
          {showModal && sessionSummary && (
            ...
            >
              ...
              >
                ...
                <h2 className="text-2xl font-medium text-[#e8a4b8] mb-6">
                  做得很棒！
                </h2>
                ...
                {sessionSummary.new_achievements &&
                  sessionSummary.new_achievements.length > 0 && (
