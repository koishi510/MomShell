...
7. **知识问答直接回答**：当她询问事实性、知识性问题（如育儿方法、健康常识、营养搭配、生活技巧、学科知识等），直接给出准确、具体、有用的回答，不要用情感引导或鼓励来替代信息提供。此时回复可以超过3句话，确保信息完整。
%s
...
7. **知识问答直接回答**：当他询问事实性、知识性问题（如育儿方法、健康常识、家务技巧、学科知识等），直接给出准确、具体、有用的回答，不要用引导性提问来替代信息提供。此时回复可以超过3句话，确保信息完整。
%s
...
7. **知识问答直接回答**：当对方询问事实性、知识性问题，直接给出准确、具体的回答。可以在更高专业层次上讨论，不需要回避知识性内容。此时回复可以超过3句话，确保信息完整。
%s
...
func (s *ChatService) chatAuthenticated(ctx context.Context, msg dto.UserMessage, userID string) (*dto.VisualResponse, error) {
	...
	// Cross-validate: second LLM call for structure + safety review
	vr, cvErr := llmvalidate.CrossValidate(ctx, s.client, llmvalidate.TypeChatResponse, rawContent, webResults)
	if cvErr != nil {
		log.Printf("[ChatService] cross-validate error: %v", cvErr)
	}
	if vr != nil && !vr.Valid {
		log.Printf("[ChatService] chat response structure invalid: %v", vr.StructErrors)
	}

	parsed := parseLLMResponse(rawContent)

	// Apply safety appendix to text if needed
	if text, ok := parsed["text"].(string); ok && vr != nil {
		parsed["text"] = llmvalidate.ApplyAppendix(text, vr)
	}
...
func (s *ChatService) chatGuest(ctx context.Context, msg dto.UserMessage) (*dto.VisualResponse, error) {
	...
	// Cross-validate: second LLM call for structure + safety review
	guestVR, cvErr := llmvalidate.CrossValidate(ctx, s.client, llmvalidate.TypeChatResponse, rawContent, webResults)
	if cvErr != nil {
		log.Printf("[ChatService] guest cross-validate error: %v", cvErr)
	}
	if guestVR != nil && !guestVR.Valid {
		log.Printf("[ChatService] guest chat response structure invalid: %v", guestVR.StructErrors)
	}

	parsed := parseLLMResponse(rawContent)

	// Apply safety appendix to text if needed
	if text, ok := parsed["text"].(string); ok && guestVR != nil {
		parsed["text"] = llmvalidate.ApplyAppendix(text, guestVR)
	}
...
func (s *ChatService) generateAndSaveSummary(userID string, existingSummary string, oldTurns []map[string]interface{}) {
	...
	newSummary = llmvalidate.Sanitize(newSummary)

	// Cross-validate summary content safety
	svr, cvErr := llmvalidate.CrossValidate(ctx, s.client, llmvalidate.TypePlainText, newSummary, "")
	if cvErr != nil {
		log.Printf("[ChatService] summary cross-validate error: %v", cvErr)
	}
	if svr != nil {
		newSummary = llmvalidate.ApplyAppendix(newSummary, svr)
	}
