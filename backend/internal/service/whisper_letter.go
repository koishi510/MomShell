...
func (s *WhisperService) GetFutureLetterView(callerID string) (*dto.FutureLetterView, error) {
	...
	currentAgeStage := resolvedFutureLetterAgeStage(user, s.userRepo)
...
func (s *WhisperService) RespondFutureLetter(authorID string, req dto.FutureLetterRespondRequest) (*dto.FutureLetterResponseItem, error) {
	...
	switch req.LetterCode {
	case futureLetterCodeOverview:
		return s.respondOverviewLetter(author, req)
	case futureLetterCodePrenatal:
		return s.respondPrenatalLetter(author, req)
	case futureLetterCodeVaccine:
		return s.respondVaccineLetter(author, req)
	case futureLetterCodeSafetySeat:
		return s.respondSafetySeatLetter(author, req)
	default:
		return nil, fmt.Errorf("信件版本已更新，请刷新后重试")
	}
}

func (s *WhisperService) respondOverviewLetter(author *model.User, req dto.FutureLetterRespondRequest) (*dto.FutureLetterResponseItem, error) {
	stageOpt, ok := findFutureLetterOption(overviewStageOptions, req.StageOptionID)
	if !ok {
		return nil, fmt.Errorf("阶段选项无效")
	}
	stateOpt, ok := findFutureLetterOption(overviewStateOptions, req.StateOptionID)
	if !ok {
		return nil, fmt.Errorf("状态选项无效")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("心语工单生成失败，请稍后再试")
	}

	if err := s.syncFamilyAgeStage(author, stageOpt.Tag); err != nil {
		return nil, err
	}
	if err := s.syncDadMissionTasks(*author.PartnerID, author.ID, mission); err != nil {
		return nil, fmt.Errorf("心语工单生成失败，请重试")
	}
	...
	record := buildFutureLetterResponseRecord(
		...
		stateOpt.Label,
		mission,
		buildFutureLetterImagePrompt(req.LetterCode, stageOpt.Label, stateOpt.Label, mission.ImageTone, wish),
		wish,
	)
...
func (s *WhisperService) respondPrenatalLetter(author *model.User, req dto.FutureLetterRespondRequest) (*dto.FutureLetterResponseItem, error) {
	progressOpt, ok := findFutureLetterOption(prenatalProgressOptions, req.StageOptionID)
	if !ok {
		return nil, fmt.Errorf("产检进度选项无效")
	}
	supportOpt, ok := findFutureLetterOption(prenatalSupportOptions, req.StateOptionID)
	if !ok {
		return nil, fmt.Errorf("产检支持选项无效")
	}
	...
	record := buildFutureLetterResponseRecord(
		...
		supportOpt.Label,
		mission,
		buildFutureLetterImagePrompt(req.LetterCode, progressOpt.Label, supportOpt.Label, mission.ImageTone, wish),
		wish,
	)
...
func (s *WhisperService) respondVaccineLetter(author *model.User, req dto.FutureLetterRespondRequest) (*dto.FutureLetterResponseItem, error) {
	progressOpt, ok := findFutureLetterOption(vaccineProgressOptions, req.StageOptionID)
	if !ok {
		return nil, fmt.Errorf("疫苗进度选项无效")
	}
	supportOpt, ok := findFutureLetterOption(vaccineSupportOptions, req.StateOptionID)
	if !ok {
		return nil, fmt.Errorf("疫苗困扰选项无效")
	}
...
func (s *WhisperService) respondSafetySeatLetter(author *model.User, req dto.FutureLetterRespondRequest) (*dto.FutureLetterResponseItem, error) {
	progressOpt, ok := findFutureLetterOption(safetySeatProgressOptions, req.StageOptionID)
	if !ok {
		return nil, fmt.Errorf("安全座椅阶段选项无效")
	}
	supportOpt, ok := findFutureLetterOption(safetySeatSupportOptions, req.StateOptionID)
	if !ok {
		return nil, fmt.Errorf("安全座椅支持选项无效")
	}
...
func buildFutureLetterResponseRecord(
	...
	tasksJSON, _ := json.Marshal(mission.Tasks)
	record := &model.FutureLetterResponse{
		...
		DadPlanCode:  mission.Code,
		DadPlanTitle: mission.Title,
		DadHeadline:  mission.Headline,
		DadSummary:   mission.Summary,
		DadTasksJSON: string(tasksJSON),
...
func (s *WhisperService) buildDadMission(
	...
	ageStage := resolvedFutureLetterAgeStage(author, s.userRepo)
...
你的任务是：读取妈妈今天在心语情报问卷中的选择、她补充的心愿、她与 AI 的聊天记忆、过往心愿记录和历史情报，把这些信息转译成爸爸今天该执行的工单。

严格要求：
- 必须优先回应妈妈今天这份情报里的真实诉求，不能忽视当前主题
- 必须贴合宝宝当前年龄阶段，不能脱离年龄乱提建议
- 输出 2 到 4 个任务，其中第 1 个任务必须直接回应今天这封信；其余任务可以结合妈妈的心愿、聊天记忆和年龄阶段补充
- 每个任务都必须是“大的关键词 + 具体行动”结构
- title 要短、像任务关键词，4 到 12 个中文字符即可
- description 只写具体、操作性强的动作，不要写空话、概念话
- 允许出现“预约整理、排队陪同、资料收纳、上车检查、夜间补给、流程接管、留观记录”这类现实任务
- 严禁生成技术难度高、华而不实、需要复杂厨艺/复杂制作的建议；像“制作蓝莓果酱”这类内容不要出现
- category 只能是 housework、parenting、health、emotional
- priority 只能是 T0、T1、T2
- difficulty 只能是 1 到 4 的整数
- headline 要以“【核心指令】”开头
- summary 用 1 到 2 句话解释为什么这些任务是今天最该做的
...
func resolvedFutureLetterAgeStage(
	...
	stage, _ := resolveAgeStage(user, userRepo, nil)
	return stage
}
...
func toFutureLetterResponseItem(item model.FutureLetterResponse) dto.FutureLetterResponseItem {
	tasks := make([]dto.FutureLetterTaskItem, 0)
	if item.DadTasksJSON != "" {
		var raw []dadMissionTaskDef
		if err := json.Unmarshal([]byte(item.DadTasksJSON), &raw); err == nil {
			tasks = make([]dto.FutureLetterTaskItem, 0, len(raw))
			for _, task := range raw {
				tasks = append(tasks, dto.FutureLetterTaskItem{
					Title:       task.Title,
